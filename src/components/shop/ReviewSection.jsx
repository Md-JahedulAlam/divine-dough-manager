...
import { averageRating } from "@/lib/shop";
import { StarRating } from "./StarRating";
function ReviewSection({ reviews }) {
  const avg = averageRating(reviews);
  return <section id="reviews" className="bg-blush py-20">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <div className="mb-12 text-center">
          <p className="eyebrow mb-3">Sweet Words</p>
          <h2 className="section-title">What Our Customers Say</h2>
          {reviews.length > 0 && <div className="mt-5 inline-flex items-center gap-3 rounded-full bg-card px-5 py-2.5 shadow-card">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {reviews.slice(0, 6).map((r) => <figure key={r.id} className="card-lift relative rounded-3xl bg-card p-6">
