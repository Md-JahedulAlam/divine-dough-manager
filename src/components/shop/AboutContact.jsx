  ...
  />
        ...
        <div>
          ...
          <ul className="mt-8 grid grid-cols-2 gap-4 text-sm">
            {["Baked fresh daily", "Custom designs", "Same-day delivery", "Eggless options"].map((t) => <li key={t} className="flex items-center gap-2 font-semibold text-chocolate">
...
function ContactSection() {
  return <section id="contact" className="bg-blush py-20">
          ...
          <div className="space-y-4 lg:col-span-2">
            ...
            <div className="rounded-3xl bg-card p-5 shadow-card">
              ...
              <ul className="mt-3 space-y-1.5 text-sm">
                {SHOP.hours.map((h) => <li key={h.day} className="flex justify-between">
...
function InfoCard({ icon, title, lines }) {
  return <div className="flex items-start gap-3 rounded-3xl bg-card p-5 shadow-card">
...
function Footer() {
  return <footer className="bg-chocolate py-10 text-chocolate-foreground">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-5 text-sm sm:flex-row lg:px-8">
        ...
        <p className="opacity-70">© {(/* @__PURE__ */ new Date()).getFullYear()} {SHOP.name}. Baked with love in Dhaka.</p>
      </div>
    </footer>;
}
export {
