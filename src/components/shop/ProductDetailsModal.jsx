...
function ProductDetailsModal({
  ...
  return <Dialog open={!!product} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl p-0 sm:max-w-3xl">
        {product && <div className="grid md:grid-cols-2">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-6">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {product.sizes.map((s) => <span key={s} className="rounded-full bg-secondary px-3 py-1 text-sm font-semibold text-secondary-foreground">
                      {s}
                    </span>)}
                </div>
              </div>
              ...
              <div className="mt-8">
                ...
                {productReviews.length === 0 ? <p className="mt-2 text-sm text-muted-foreground">No reviews yet for this cake.</p> : <ul className="mt-3 space-y-4">
                    {productReviews.map((r) => <li key={r.id} className="rounded-2xl bg-cream p-4">
