...
export function CategorySection({
  ...
  const coverFor = (c: Category) => products.find((p) => p.category_id === c.id && p.image_url)?.image_url;
  const countFor = (c: Category) => products.filter((p) => p.category_id === c.id).length;

  return (
    <section id="categories" className="bg-blush py-20">
